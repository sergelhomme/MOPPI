...
class Moppi:
  ...
  #----------------------------------------------------------------------------------------------------------------------------------------                 Geocodage            -----------------------------------------------------------------------------------------------------------------------------------
	...
	def geocodcal(self):
		...
		vln.startEditing()
		prn.addAttributes( [QgsField("ID", QVariant.Int), QgsField("Adresse", QVariant.String), QgsField("Long", QVariant.Double), QgsField("Lat", QVariant.Double), QgsField("Nbr", QVariant.Int)] )
		vln.commitChanges()
		for i in range(len(tabadrr)):
			...
			try :
				...
				fet.setAttributes( [ int(tabid[i]) , str(tabadrr[i]), float(x), float(y), int(1) ] )
				vln.startEditing()
				prn.addFeatures( [ fet ] )
				vln.commitChanges()
			except :
					...
					if ok2 :
						...
						for i in range(len(prb_err)):
							try :
								...
								pointCRSdest = xform.transform(QgsPoint(float(x), float(y)))
								fet.setGeometry( QgsGeometry.fromPoint(pointCRSdest) )
								fet.setAttributes( [ int(prb_err[i][0]) , str(prb_err[i][1]), float(x), float(y), int(1) ] )
								vln.startEditing()
								prn.addFeatures( [ fet ] )
								vln.commitChanges()
							except :
